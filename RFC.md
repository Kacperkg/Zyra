...
| Authentication requirements | Access tokens around one day, refresh tokens around one week, and password recovery |
...
Token storage and format, password hashing, refresh behaviour, recovery implementation, mailbox access, file storage, production database setup, background processing, caching, queues, security, and operations remain undecided. The agreed choices do not select additional frameworks or infrastructure.
...
| Authentication | What token format/storage, hashing, refresh behaviour, and recovery mechanism will be used? |
